...
with st.sidebar:
    ...
    if st.button("Login"):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Hier erwarten wir: access_token und role
                ...
                st.error(f"❌ Login fehlgeschlagen – bitte prüfen!\nStatus: {resp.status_code}\nText: {resp.text}")
        except Exception as e:
...
if role == "guest" or role == "user":
    st.markdown("## 🎬 Empfehlungen für Gäste")
    try:
...
# === ADMIN-Bereich ===
    ...
    st.markdown("## 👑 Admin Panel & Empfehlungen")
